        ...
        if in_coverage_section:
            ...
            if len(parts) >= 4:
                try:
                    ...
                    coverage_data.append((file_path, total_lines, missed_lines, coverage_percent))
...
def analyze_coverage(coverage_data: List[Tuple[str, int, int, float]], 
                    limit: int = 10, 
                    min_lines: int = 10) -> None:
    ...
    overall_coverage = ((total_lines - total_missed) / total_lines * 100) if total_lines > 0 else 0
    ...
    for i, (file_path, total_lines, missed_lines, coverage_pct) in enumerate(lowest_coverage[:limit]):
        ...
        print(f"{display_path:<50} {coverage_indicator} {coverage_pct:>6.1f}% {total_lines:>6} {missed_lines:>6}")
    ...
    print(f"\n💡 Recommendations:")
    if zero_coverage:
        print(f"   1. Start with zero-coverage files - easiest wins!")
    
    lowest_non_zero = [item for item in lowest_coverage if item[3] > 0][:3]
    if lowest_non_zero:
        print(f"   2. Focus on these low-coverage files:")
        for file_path, _, _, coverage_pct in lowest_non_zero:
            module_name = file_path.replace("src/goldentooth_agent/", "").replace(".py", "").replace("/", ".")
    ...
    if high_impact:
        print(f"   3. High-impact opportunities (many uncovered lines):")
        for file_path, total_lines, missed_lines, coverage_pct in high_impact:
            module_name = file_path.replace("src/goldentooth_agent/", "").replace(".py", "").replace("/", ".")
            impact_score = missed_lines
            print(f"      • {module_name} ({missed_lines} uncovered lines, {coverage_pct:.1f}%)")


def main():
    parser = argparse.ArgumentParser(description="Analyze test coverage and identify lowest coverage files")
    parser.add_argument("--limit", type=int, default=15, help="Number of lowest coverage files to show")
    parser.add_argument("--min-lines", type=int, default=10, help="Minimum lines to consider a file significant")
