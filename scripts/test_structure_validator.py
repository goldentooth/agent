...
class TestStructureValidator:
    ...
    def validate_structure(self) -> ValidationResult:
        ...
        for test_file in test_files:
            ...
            if expected_source.exists():
                ...
                if missing_symbols and len(missing_symbols) > 2:  # Allow some flexibility
                    rel_test = test_file.relative_to(self.test_root)
    ...
    def print_validation_results(self, result: ValidationResult, show_details: bool = True):
        ...
        coverage_pct = (result.valid_mappings / result.total_source_files * 100) if result.total_source_files > 0 else 0
        print(f"   • Test coverage ratio: {coverage_pct:.1f}%")
        
        # Issues summary
        total_issues = len(result.orphaned_tests) + len(result.missing_tests) + len(result.misaligned_tests)
        ...
        print(f"\n💡 Recommendations:")
        if result.missing_tests:
            print(f"   1. Create missing test files for better coverage")
        if result.orphaned_tests:
            print(f"   2. Review orphaned tests - rename, move, or remove them")
        if result.misaligned_tests:
            print(f"   3. Check misaligned tests for outdated imports/references")


def main():
    parser = argparse.ArgumentParser(description="Validate test structure alignment with source code")
    ...
    if args.json:
        ...
        json_result = {
            ...
            "misaligned_tests": [{"file": f, "issue": i} for f, i in result.misaligned_tests],
            "valid_mappings": result.valid_mappings,
            "total_source_files": result.total_source_files,
            "total_test_files": result.total_test_files,
            "issues_found": len(result.orphaned_tests) + len(result.missing_tests) + len(result.misaligned_tests)
        }
