...
R = TypeVar("R", bound=ContextFlowSchema)
...
class FlowTool:
    ...
    def to_agent(self) -> AnyAgent:  # Use Any to avoid forward reference issues
        ...
        return FlowAgent(
            ...
            processing_flow=self.to_flow(),
        )

    async def _tool_pipeline(
        self, stream: AsyncGenerator[AnyInput, None]  # Use Any for flexibility
    ) -> AsyncGenerator[AnyOutput, None]:  # Use Any for flexibility
        """Implementation of the tool pipeline."""
        async for input_data in stream:
            # Handle Context objects from agent pipeline
            from context import Context

            if isinstance(input_data, Context):
                # This is a Context object, extract the input schema
                try:
                    validated_input = self.input_schema.from_context(input_data)
                except (KeyError, ValueError):
                    # If we can't extract from context, create a minimal fallback
                    raise ValueError(
                        f"Could not extract {self.input_schema.__name__} from context"
                    )
            ...
            # If input was a Context, convert output back to Context
            if isinstance(input_data, Context):
                from context import Context

                yield validated_output.to_context(input_data)
            else:
                yield validated_output

    async def _passthrough(
        self, stream: AsyncGenerator[AnyInput, None]
    ) -> AsyncGenerator[AnyOutput, None]:
