...
class TokenUsageRecord(BaseModel):
    ...
    operation_type: str = Field(..., description="Type of operation (embed, re_embed, skip)")
...
class TokenBudget(BaseModel):
    ...
    warning_threshold: float = Field(default=0.8, description="Warning at 80% of budget")
...
class TokenTracker:
    ...
    def _init_database(self) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_operation_type ON token_usage(operation_type)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_model_name ON token_usage(model_name)")
            
            conn.commit()
    
    def count_tokens(self, text: str, model_name: str = "text-embedding-ada-002") -> int:
        """
    ...
    def _calculate_cost(self, token_count: int, model_name: str) -> float:
        ...
        rate = pricing_per_1k_tokens.get(model_name, 0.0001)  # Default to ada-002 pricing
    ...
    def _store_record(self, record: TokenUsageRecord) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                record.operation_id, record.operation_type, record.timestamp,
                record.content_hash, record.content_length, record.token_count,
                record.model_name, record.model_version, record.estimated_cost_usd,
                record.document_type, record.module_path, record.codebase_name,
                record.was_cached, record.change_reason
            ))
            conn.commit()
    ...
    def _update_budget(self, tokens_used: int) -> None:
        ...
        if self.budget.last_reset_date != today:
            last_reset = datetime.fromisoformat(self.budget.last_reset_date) if self.budget.last_reset_date else now
    ...
    def _save_budget(self) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
                ...
                VALUES (1, ?, ?, ?, ?, ?, ?)
            """, (
                self.budget.daily_limit, self.budget.monthly_limit,
                self.budget.warning_threshold, self.budget.daily_used,
                self.budget.monthly_used, self.budget.last_reset_date
            ))
            conn.commit()
    
    def check_budget_status(self) -> dict[str, Any]:
        """Check current budget status and warnings."""
        daily_usage_pct = self.budget.daily_used / self.budget.daily_limit if self.budget.daily_limit > 0 else 0
        monthly_usage_pct = self.budget.monthly_used / self.budget.monthly_limit if self.budget.monthly_limit > 0 else 0
    ...
    def get_usage_statistics(self, days: int = 30) -> dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            by_operation = {row[0]: (row[1], row[2], row[3]) for row in cursor.fetchall()}
    ...
    def get_cost_breakdown(self, group_by: str = "day", days: int = 30) -> dict[str, Any]:
