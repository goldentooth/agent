...
class IntrospectionQuery(BaseModel):
    ...
    codebases: Optional[list[str]] = Field(default=None, description="Specific codebases to search")
    document_types: Optional[list[CodebaseDocumentType]] = Field(default=None, description="Document types to include")
    limit: int = Field(default=10, description="Maximum results")
    include_source: bool = Field(default=True, description="Include source code in results")
...
class IntrospectionResult(BaseModel):
    ...
    codebases_searched: list[str] = Field(..., description="Codebases that were searched")
    ...
    def get_code_snippets(self) -> list[str]:
        ...
        for result in self.results:
            if result.get("metadata", {}).get("document_type") in [
                "source_code", "function_definition", "class_definition"
            ]:
...
class CodebaseComparison(BaseModel):
    ...
    similarities: dict[str, list[dict[str, Any]]] = Field(..., description="Similar patterns by codebase")
...
class CodebaseIntrospectionService:
    ...
    async def compare_codebases(
        ...
        for codebase_name in codebase_names:
            ...
            if codebase_results:
                ...
                doc_types = [r["metadata"].get("document_type") for r in codebase_results]
                complexity_scores = [r["metadata"].get("complexity_score", 0) for r in codebase_results]
    ...
    def _infer_document_types(self, query: str) -> list[CodebaseDocumentType]:
        ...
        type_keywords = {
            CodebaseDocumentType.FUNCTION_DEFINITION: ["function", "method", "def", "callable"],
            CodebaseDocumentType.CLASS_DEFINITION: ["class", "object", "type", "inheritance"],
            CodebaseDocumentType.MODULE_BACKGROUND: ["design", "architecture", "motivation", "theory", "background"],
            CodebaseDocumentType.MODULE_API: ["api", "interface", "usage", "example", "how to"],
            CodebaseDocumentType.SOURCE_CODE: ["implementation", "code", "source", "algorithm"],
    ...
    async def _find_related_source(self, metadata: dict[str, Any]) -> str | None:
        ...
        try:
            ...
            if result.results:
                return result.results[0]["content"][:500] + "..."  # Truncate for display
        except Exception:
            pass
    ...
    async def get_codebase_overview(self, codebase_name: str) -> dict[str, Any]:
        ...
        arch_query = IntrospectionQuery(
            ...
            codebases=[codebase_name],
            document_types=[CodebaseDocumentType.MODULE_BACKGROUND, CodebaseDocumentType.MODULE_API],
            limit=5
        )
        ...
        components_query = IntrospectionQuery(
            ...
            codebases=[codebase_name],
            document_types=[CodebaseDocumentType.CLASS_DEFINITION, CodebaseDocumentType.MODULE_API],
            limit=10
        )
