...
class CodebaseDocumentExtractor:
    ...
    async def extract_from_path(
        ...
        for file_path in self._walk_files(root_path, include_patterns, exclude_patterns):
    ...
    async def _extract_from_python_file(
        ...
        if module_docstring:
            ...
            documents.append(CodebaseDocument(
                document_id=doc_id,
    ...
    def _extract_function_document(
        ...
        return CodebaseDocument(
            ...
            content=func_source,
            summary=func_docstring.split('\n')[0] if func_docstring else f"Function {node.name}",
            file_path=str(file_path),
    ...
    def _extract_class_document(
        ...
        return CodebaseDocument(
            ...
            summary=class_docstring.split('\n')[0] if class_docstring else f"Class {node.name}",
            file_path=str(file_path),
            line_start=node.lineno,
            line_end=node.end_lineno or node.lineno,
            module_path=module_path,
