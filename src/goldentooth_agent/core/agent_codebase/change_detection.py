...
class ContentFingerprint(BaseModel):
    ...
    embedding_id: str | None = Field(
        default=None, description="Vector store embedding ID"
    )
    ...
    def needs_re_embedding(self, new_fingerprint: ContentFingerprint) -> bool:
        ...
        if new_fingerprint.file_mtime > self.file_mtime and self.has_structure_changed(
            new_fingerprint
        ):
            return True
...
class ChangeDetectionIndex(BaseModel):
    ...
    def get_fingerprint(self, document_id: str) -> ContentFingerprint | None:
...
class SmartChangeDetector:
    ...
    def analyze_document_changes(
        self, document: CodebaseDocument, file_path: Path
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef):
                    ...
                    structure_elements.append(
                        f"function:{node.name}:{len(node.args.args)}"
                    )
                    ...
                    structure_elements.extend(
                        f"base:{self._ast_to_string(base)}" for base in node.bases
                    )
    ...
    def get_indexing_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_complexity": (
                sum(fp.complexity_score for fp in self.index.fingerprints.values())
                / len(self.index.fingerprints)
                if self.index.fingerprints
                else 0
            ),
        }
