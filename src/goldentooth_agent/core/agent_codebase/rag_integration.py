...
from .introspection import CodebaseIntrospectionService, IntrospectionQuery
...
class CodebaseRAGQuery(BaseModel):
    ...
    include_codebase: bool = Field(
        default=True, description="Include codebase introspection"
    )
    include_documents: bool = Field(
        default=True, description="Include document store search"
    )
    codebase_weight: float = Field(
        default=0.6, description="Weight for codebase results (0-1)"
    )
...
class CodebaseRAGResult(BaseModel):
    ...
    codebase_results: list[dict[str, Any]] = Field(
        default_factory=list, description="Codebase introspection results"
    )
    document_results: list[dict[str, Any]] = Field(
        default_factory=list, description="Document store results"
    )
...
class CodebaseRAGIntegration:
    ...
    async def query(self, query: CodebaseRAGQuery) -> CodebaseRAGResult:
        ...
        if query.include_codebase:
            codebase_query = IntrospectionQuery(
                query=query.query, limit=int(query.max_results * query.codebase_weight)
            )
        ...
        combined_results = self._combine_results(
            codebase_result, document_result, query.codebase_weight
        )
                ...
                formatted_results.append(
                    {
                        ...
                        "source": {
                            ...
                            "source_name": doc.get("metadata", {}).get(
                                "source_name", ""
                            ),
                        },
                    }
    ...
    def _combine_results(
        ...
        for result in codebase_result.get("results", []):
            ...
            weighted_result["weighted_score"] = (
                result.get("score", 0.0) * codebase_weight
            )
        ...
        for result in document_result.get("results", []):
            weighted_result = result.copy()
            weighted_result["weighted_score"] = (
                result.get("score", 0.0) * document_weight
            )
...
Please provide a clear, practical answer that combines conceptual understanding with concrete examples where relevant."""
    ...
    def _extract_sources(
        self, codebase_result: dict[str, Any], document_result: dict[str, Any]
    ) -> list[str]:
    ...
    async def get_codebase_overview(self) -> dict[str, Any]:
        ...
        return await self.introspection_service.get_codebase_overview(
            "goldentooth_agent"
        )
