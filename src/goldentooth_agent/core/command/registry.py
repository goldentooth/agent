...
class CommandRegistry:
    ...
    def __init__(self, logger=inject[get_logger(__name__)]) -> None:
    ...
    @inject.method
    def enroll(
        self, callable: Callable[[], None], logger=inject[get_logger(__name__)]
    ) -> None:
        """Enroll a comand."""
    ...
    @inject.method
    def clear(self, logger=inject[get_logger(__name__)]):
        """Clear all registered command registrars."""
        logger.debug(f"Clearing {len(self.commands)} commands")
        self.commands.clear()

    @inject.method
    def register(self, logger=inject[get_logger(__name__)]):
