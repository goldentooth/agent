...
class ToolAgent(BaseAgent):
    ...
    @inject
    def __init__(self, tool: BaseTool, logger=inject[get_logger(__name__)]):
    ...
    @inject
    def get_response(
        self, response_model=None, logger=inject[get_logger(__name__)]
    ) -> type[BaseModel]:
        """Get the response from the tool agent."""
        logger.debug(
            f"Getting response from ToolAgent with tool: {self.tool.tool_name}"
        )
    ...
    def run(
        ...
        logger.debug(
            f"Running ToolAgent with tool: {self.tool.tool_name} and user input: {user_input}"
        )
        if user_input:
        ...
        if not isinstance(user_input, self.input_schema):
            raise TypeError(
                f"Expected input of type {self.input_schema}, got {type(user_input)}"
            )
    ...
    async def run_async(  # type: ignore[override]
        ...
        logger.debug(
            f"Running ToolAgent asynchronously with tool: {self.tool.tool_name} and user input: {user_input}"
        )
        if user_input:
            self.memory.initialize_turn()
            self.current_user_input = user_input
