...
def thunkify_agent(agent: BaseAgent) -> Thunk[BaseIOSchema, BaseIOSchema]:
    ...
    @inject
    async def _thunkify_agent(
        params: BaseIOSchema, logger=inject[get_logger(__name__)]
    ) -> BaseIOSchema:
...
def set_should_skip_agent_key(value: bool = True) -> Thunk[Context, Context]:
    ...
    @inject
    async def _set_should_exit(
        ctx: Context, logger=inject[get_logger(__name__)]
    ) -> Context:
...
def should_skip_agent() -> Callable[[Context], bool]:
    ...
    @inject
    def _should_skip_agent(ctx: Context, logger=inject[get_logger(__name__)]) -> bool:
        """Check if the agent should be skipped."""
...
def enable_agent_context_provider(
    agent_name: str, agent_fn: Callable[[], str]
...
def inject_agent_prefix() -> Thunk[Context, Context]:
    ...
    ) -> Annotated[Text, AGENT_PREFIX_KEY]:
        """Inject the agent's text representation into the context."""
