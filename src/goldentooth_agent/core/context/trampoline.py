...
def set_should_break(
    value: bool = True, logger=inject[get_logger(__name__)]
...
async def trampoline(
    ctx: Context, thunk: Thunk[Context, Context], logger=inject[get_logger(__name__)]
) -> Context:
    ...
    while True:
        ...
        if ctx.get_or_default(SHOULD_BREAK_KEY, lambda: False):
            logger.debug(
                "Break requested, resetting should_break flag and restarting the loop."
            )
...
@inject
def exitable_chain(
    *steps: Thunk[Context, Context], logger=inject[get_logger(__name__)]
) -> Thunk[Context, Context]:
