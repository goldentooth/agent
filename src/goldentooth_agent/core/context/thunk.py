...
def clear_context_keys(
    keys: list[ContextKey[Any]],
) -> Callable[[Thunk[Context, Context]], Thunk[Context, Context]]:
...
def context_autothunk(
    *, name: str
) -> Callable[[Callable[..., Any]], Thunk[Context, Context]]:
...
def set_context_key(key: ContextKey, value: Any) -> Thunk[Context, Context]:
    """Thunk to set a context key to a specific value."""

    @thunk(name=f"set_context_key({key.name} = {value})")
    async def _set_context_key(ctx: Context) -> Context:
        """Set the context key to the specified value."""
        ctx.set(key, value)
        return ctx
...
def has_context_key_value(
    ...
    def _has_context_key_value(ctx: Context) -> bool:
        ...
        logger.debug(
            f"Checking if context has key {key.name} with value {value} (default: {default})"
        )
        if ctx.has(key):
            current_value = ctx.get_or_default(key, lambda: default)
            result = current_value == value
            logger.debug(
                f"Context key {key.name} has value {current_value}, expected {value}: {result}"
            )
            return result
        else:
            logger.debug(
                f"Context key {key.name} is not set, returning default: {default}"
            )
