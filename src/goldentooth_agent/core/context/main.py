...
class Context:
    ...
    def get(self, key: ContextKey[T]) -> T:
        ...
        if not isinstance(val, key.type_T):
            raise TypeError(
                f"{key.name} wrong type; expected {key.type_T.__name__}, got {type(val).__name__}"
            )
        return val
    ...
    def set(self, key: ContextKey[T], value: T) -> None:
        ...
        if not isinstance(value, key.type_T):
            raise TypeError(
                f"Value for {key.name} must be of type {key.type_T.__name__}, got {type(value).__name__}"
            )
