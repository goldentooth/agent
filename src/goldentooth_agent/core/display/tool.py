...
class DisplayInput(BaseIOSchema):
    ...
    output: Any = Field(
        ...,
        description="The output to display to the user. For example, 'Hello, World!'.",
    )
...
class DisplayTool(BaseTool):
    ...
    def __init__(
        ...
        config: DisplayConfig = DisplayConfig(
            title="tools.display", description="Displays some output to the user."
        ),
    ):
    ...
    @inject
    def run(self, params: DisplayInput, console: Console = inject[get_console()], logger=inject[get_logger(__name__)]) -> DisplayOutput:  # type: ignore[override]
