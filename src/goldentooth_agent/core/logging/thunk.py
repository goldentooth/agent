...
def log_thunk(
    ...
    return Thunk(
        _log,
        name=f"log_thunk({name}, {logging.getLevelName(level)})",
        metadata={"fmt": fmt},
    )
...
def change_log_level(logger_name: str, level: int = logging.INFO) -> Thunk[Any, Any]:
    """Change the log level of a logger."""

    @inject
    async def _change_log_level(
        _, logger: Logger = inject[get_logger(logger_name)]
    ) -> Any:
        """Change the log level of the specified logger."""
        logger.debug(
            f"Changing log level for {logger_name} to {logging.getLevelName(level)}"
        )
        logger.setLevel(level)
        logger.debug(
            f"Log level for {logger_name} changed to {logging.getLevelName(level)}"
        )
        return _

    return Thunk(
        _change_log_level,
        name=f"change_log_level({logger_name}, {logging.getLevelName(level)})",
    )
...
def with_log_level(
    ...
    @inject
    def _decorator(
        fn: Thunk[TIn, TOut], logger: Logger = inject[get_logger(logger_name)]
    ) -> Thunk[TIn, TOut]:
