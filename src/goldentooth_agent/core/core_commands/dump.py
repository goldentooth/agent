...
def enroll_dump_command(
    ...
    def _command(
        ...
        thing: Annotated[
            Optional[str],
            typer.Argument(help="Name of the thing to dump, e.g. 'context'"),
        ] = None,
    ) -> None:
        """Dump the context."""
...
def get_dumpables() -> dict[str, Dumpable]:
    ...
    from goldentooth_agent.core.agent_config import (
        AgentConfigRegistry,
    ...
    from goldentooth_agent.core.context_provider import (
    ...
    from goldentooth_agent.core.scenario_selector import (
        ScenarioSelectorStrategyRegistry,
    )
    from goldentooth_agent.core.system_prompt import (
    ...
    result["agent_config_store"] = world[YamlAgentConfigStore]
    ...
    result["persona_selector_strategy_registry"] = world[
    ...
    result["scenario_selector_strategy_registry"] = world[
        ScenarioSelectorStrategyRegistry
    ]
...
def dumpable_from_string(thing: str, values: dict[str, Dumpable]) -> Dumpable:
    ...
    if thing in values:
        ...
        raise ValueError(
            f"Unknown dumpable: {thing}. Available dumpables: {', '.join(values.keys())}"
        )
