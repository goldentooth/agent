...
def install_sample_data(paths: Paths) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            org_files = importlib.resources.files(sample_data_package).joinpath(
                "github/orgs"
            )
            if org_files.is_dir():
        ...
        try:
            repo_files = importlib.resources.files(sample_data_package).joinpath(
                "github/repos"
            )
        ...
        # Install notes
        try:
            note_files = importlib.resources.files(sample_data_package).joinpath(
                "notes"
            )
