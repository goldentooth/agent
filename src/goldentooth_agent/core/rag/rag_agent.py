...
class RAGInput(FlowIOSchema):
    ...
    max_results: int = Field(default=10, description="Maximum number of documents to retrieve")
    ...
    domain_context: str | None = Field(default=None, description="Optional domain context for query expansion")
...
class RAGOutput(FlowIOSchema):
    ...
    query_analysis: dict[str, Any] = Field(
        ...
        description="Analysis of the user's query"
    )
...
class RAGAgent:
    ...
    async def _system_pipeline(self, stream: AsyncIterator[Context]) -> AsyncIterator[Context]:
        """System pipeline for preparing RAG context."""
        async for context in stream:
            # Extract input data
            try:
                question = context["schema.RAGInput.question"]
                conversation_history = context.get("schema.RAGInput.conversation_history", [])
    ...
    async def _processing_pipeline(self, stream: AsyncIterator[Context]) -> AsyncIterator[Context]:
        ...
        async for context in stream:
            try:
                ...
                if conversation_context:
                    enhanced_question = f"Context: {conversation_context}\n\nQuestion: {question}"
                ...
                response = rag_result.get("response", "I couldn't find relevant information to answer your question.")
                ...
                self._store_results_in_context(
                    context, response, sources, confidence, query_analysis, suggestions, rag_result
                )
    ...
    def _generate_suggestions(
        ...
        if not documents:
            ...
            suggestions.append("Your query was very specific - try broadening it for more results")
    ...
    def _store_results_in_context(
        ...
        response_key = ContextKey.create("schema.RAGOutput.response", str, "Agent response")
        sources_key = ContextKey.create("schema.RAGOutput.sources", list, "Source documents")
        confidence_key = ContextKey.create("schema.RAGOutput.confidence", float, "Response confidence")
        analysis_key = ContextKey.create("schema.RAGOutput.query_analysis", dict, "Query analysis")
        suggestions_key = ContextKey.create("schema.RAGOutput.suggestions", list, "Query suggestions")
        metadata_key = ContextKey.create("schema.RAGOutput.metadata", dict, "Response metadata")
        ...
        updated_context.set(metadata_key.path, {
            "rag_metadata": rag_result.get("metadata", {}),
            "num_sources": len(sources),
            "processing_time": rag_result.get("metadata", {}).get("processing_time", 0),
        })
    ...
    def _store_error_in_context(self, context: Context, error_response: str) -> None:
        ...
        updated_context.set(suggestions_key.path, ["Please try rephrasing your question"])
