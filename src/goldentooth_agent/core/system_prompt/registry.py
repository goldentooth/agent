...
class SystemPromptRegistry(NamedRegistry[SystemPromptGenerator]):
    ...
    def dump(self, logger=inject[get_logger(__name__)]) -> Table:
        ...
        for id, generator in self.items():
            generator_dict = {
                "background": generator.background if generator.background else None,
                "steps": generator.steps if generator.steps else None,
                "output_instructions": (
                    generator.output_instructions
                    if generator.output_instructions
                    else None
                ),
                "context_providers": (
                    list(generator.context_providers.keys())
                    if generator.context_providers
                    else None
                ),
            }
            table.add_row(id, Pretty(generator_dict))
        return table
