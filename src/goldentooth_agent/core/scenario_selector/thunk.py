...
from goldentooth_agent.core.context import Context, context_autothunk, set_context_key
...
from .context import SCENARIO_SELECTOR_STRATEGY_KEY, DEFAULT_SCENARIO_KEY, SCENARIO_KEY
...
def inject_scenario_selector_strategy() -> Thunk[Context, Context]:
    ...
    async def _inject_scenario_selector_strategy(
        ...
        logger.debug(
            "Using default scenario selector strategy: %s", default_strategy.id
        )
...
def inject_scenario() -> Thunk[Context, Context]:
    ...
    async def _inject_scenario(
        context: Context,
        scenario_selector_strategy: Annotated[
            ScenarioSelectorStrategy, SCENARIO_SELECTOR_STRATEGY_KEY
        ],
        ...
        logger.debug(
            "Selecting scenario using strategy: %s", scenario_selector.strategy_id
        )
...
def select_scenario_chain() -> Thunk[Context, Context]:
    ...
    return compose_chain(
        set_context_key(DEFAULT_SCENARIO_KEY, value="goldentooth"),
