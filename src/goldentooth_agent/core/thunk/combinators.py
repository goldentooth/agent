...
def map(fn: Callable[[TB], TC]) -> Callable[[Thunk[TA, TB]], Thunk[TA, TC]]:
  ...
  def wrapper(inner: Thunk[TA, TB]) -> Thunk[TA, TC]:
    ...
    async def _thunk(ctx: TA) -> TC:
      ...
      return fn(result)
    return Thunk(_thunk, name=f"{inner.name}.map({fn.__name__})")
  return wrapper
...
def flat_map(fn: Callable[[TB], Thunk[TA, TC]]) -> Callable[[Thunk[TA, TB]], Thunk[TA, TC]]:
  ...
  def wrapper(inner: Thunk[TA, TB]) -> Thunk[TA, TC]:
    ...
    return Thunk(_thunk, name=f"{inner.name}.flat_map({fn.__name__})")
  return wrapper
...
def flat_map_ctx(fn: Callable[[TB, TA], Thunk[TA, TC]]) -> Callable[[Thunk[TA, TB]], Thunk[TA, TC]]:
  ...
  def wrapper(inner: Thunk[TA, TB]) -> Thunk[TA, TC]:
    ...
    return Thunk(_thunk, name=f"{inner.name}.flat_map_ctx({fn.__name__})")
  return wrapper
...
def if_else(
  ...
  from goldentooth_agent.core.logging import get_logger
  @inject
  async def _thunk(ctx: TIn, logger: Logger = inject[get_logger(__name__)]) -> TOut:
    """Execute if_thunk if predicate is True, else execute else_thunk."""
    logger.debug(f"if_else: {predicate.__name__}({ctx})")
    pred_result = predicate(ctx)
    logger.debug(f"if_else: predicate result = {pred_result}")
    if pred_result:
      logger.debug(f"if_else: executing if_thunk {if_thunk.name}")
      return await if_thunk(ctx)
    else:
      logger.debug(f"if_else: executing else_thunk {else_thunk.name}")
      return await else_thunk(ctx)
  return Thunk(_thunk, name=f"if_else({predicate.__name__}, {if_thunk.name}, {else_thunk.name})")
...
def tap(fn: Callable[[TOut], Awaitable[None]]) -> Callable[[Thunk[TIn, TOut]], Thunk[TIn, TOut]]:
  ...
  def wrapper(inner: Thunk[TIn, TOut]) -> Thunk[TIn, TOut]:
    ...
    async def _thunk(ctx: TIn) -> TOut:
      ...
      await fn(result)
      return result
    return Thunk(_thunk, name=f"{inner.name}.tap({fn.__name__})")
  return wrapper
  ...
  def wrapper(inner: Thunk[TIn, TOut]) -> Thunk[TIn, TOut]:
    ...
    async def _thunk(ctx: TIn) -> TOut:
      ...
      try:
        ...
        return result
    return Thunk(_thunk, name=f"{inner.name}.recover({handler.__name__})")
  return wrapper
...
def switch(
  ...
  return Thunk(_thunk, name=f"switch({selector.__name__}, {list(cases.keys())}, {default.name if default else 'None'})")
...
def race(thunks: list[Thunk[TIn, TOut]]) -> Thunk[TIn, TOut]:
  ...
  return Thunk(_thunk, name="race({})".format(", ".join(thunk.name for thunk in thunks)))
...
def lift_sync(fn: Callable[[TIn], TOut]) -> Thunk[TIn, TOut]:
  ...
  async def _thunk(ctx: TIn) -> TOut:
    ...
    return fn(ctx)
  return Thunk(_thunk, name=f"lift_sync({fn.__name__})")
