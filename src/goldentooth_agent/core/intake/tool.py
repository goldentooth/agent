...
class IntakeInput(BaseIOSchema):
    ...
    prompt: str = Field(
        ...,
        description="The prompt to display to the user. For example, 'Please enter your name:'.",
    )
    style: Optional[str] = Field(
        ...,
        description="The (optional) style to apply to the prompt. For example, 'bold blue', 'italic green', etc.",
    )
...
class IntakeTool(BaseTool):
    ...
    def __init__(
        self,
        config: IntakeConfig = IntakeConfig(
            title="tools.intake",
            description="Prompts the user for input via the console.",
        ),
    ):
