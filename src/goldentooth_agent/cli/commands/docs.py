...
from ...core.schemas.github import GitHubOrg, GitHubRepo
from ...core.schemas.goldentooth import GoldentoothNode, GoldentoothService
from ...core.schemas.notes import Note
...
@app.command("list")
def list_documents(
    store_type: Optional[str] = typer.Option(None, "--type", "-t", help="Filter by store type"),
    ...
    def handle(document_store: DocumentStore = inject.me()) -> None:
        ...
        try:
            if store_type:
                # Validate store type
                valid_types = ["github.orgs", "github.repos", "goldentooth.nodes", "goldentooth.services", "notes"]
                if store_type not in valid_types:
                    console.print(f"[red]Invalid store type. Valid types: {', '.join(valid_types)}[/red]")
                    raise typer.Exit(1)
...
@app.command("show")
def show_document(
    store_type: str = typer.Argument(..., help="Store type (e.g., github.repos, notes)"),
        ...
        try:
            ...
            if store_type == "github.orgs":
                if not document_store.github_orgs.exists(document_id):
                    console.print(f"[red]Document '{document_id}' not found in {store_type}[/red]")
                    raise typer.Exit(1)
            ...
            if raw:
                ...
                if file_path.exists():
                    ...
                    console.print(f"[cyan]Raw YAML content for {store_type}/{document_id}:[/cyan]")
...
def show_stats():
    ...
    def handle(
        ...
        try:
            ...
            vector_table.add_row("Total Embeddings", str(vector_stats["total_documents"]))
...
def manage_sidecar_files(
    ...
    def handle(vector_store: VectorStore = inject.me()) -> None:
        ...
        try:
            if action == "sync":
                with console.status("[bold green]Syncing embeddings to sidecar files..."):
                    result = vector_store.sync_sidecar_files()
                
                console.print(f"[green]✓ Synced {result['created_files']} embedding files[/green]")
                ...
                table.add_row("Tracked in metadata", str(len(metadata.get("embeddings", {}))))
                ...
                console.print("[dim]This would remove .emb.gz files that don't have corresponding YAML files[/dim]")
...
@app.command("embed")
def embed_documents(
    store_type: Optional[str] = typer.Option(None, "--type", "-t", help="Embed only specific store type"),
    force: bool = typer.Option(False, "--force", "-f", help="Re-embed existing documents"),
    dry_run: bool = typer.Option(False, "--dry-run", help="Show what would be embedded without doing it"),
):
    ...
    def handle(
        ...
        async def embed_async():
            try:
                ...
                console.print(f"[cyan]Processing {total_docs} documents for embedding...[/cyan]")
                    ...
                    for doc_id in doc_ids:
                        try:
                            ...
                            if existing and not force:
                                console.print(f"  [dim]Skipping {store_name}/{doc_id} (already embedded)[/dim]")
                                skipped += 1
                                continue
                            
                            if dry_run:
                                console.print(f"  [blue]Would embed {store_name}/{doc_id}[/blue]")
                                processed += 1
                                continue
                            ...
                            if store_name == "github.orgs":
                                ...
                                console.print(f"  [red]Unknown store type: {store_name}[/red]")
                                errors += 1
                                continue
                            
                            # Create embedding
                            console.print(f"  [green]Embedding {store_name}/{doc_id}...[/green]")
                            embedding_result = await embeddings_service.create_document_embedding(doc_data)
                            ...
                            console.print(f"  [red]Error embedding {store_name}/{doc_id}: {e}[/red]")
                            errors += 1
...
def ask_question(
    ...
    max_results: int = typer.Option(5, "--max-results", "-m", help="Maximum documents to retrieve"),
    store_type: Optional[str] = typer.Option(None, "--type", "-t", help="Filter by store type"),
    threshold: float = typer.Option(0.1, "--threshold", help="Similarity threshold (0.0-1.0)"),
            ...
            try:
                ...
                if show_sources and result["retrieved_documents"]:
                    ...
                    for i, doc in enumerate(result["retrieved_documents"], 1):
                        ...
                        console.print(f"  {i}. [dim]{doc_ref}[/dim] (score: {score:.3f})")
...
def summarize_knowledge_base(
    store_type: Optional[str] = typer.Option(None, "--type", "-t", help="Summarize specific store type"),
...
def get_document_insights(
    ...
    def handle(rag_service: RAGService = inject.me()) -> None:
        ...
        async def insights_async():
            try:
                with console.status(f"[bold green]Analyzing {store_type}/{document_id}..."):
                    result = await rag_service.get_document_insights(document_id, store_type)
                ...
                console.print(f"[bold green]Insights for {store_type}/{document_id}:[/bold green]")
