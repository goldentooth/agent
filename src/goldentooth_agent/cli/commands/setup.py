...
def initialize_system(
    sample_data: bool = typer.Option(
        True, "--sample-data/--no-sample-data", help="Install sample data"
    ),
    ...
    def handle(
        ...
        try:
            console.print(
                "[bold green]Initializing Goldentooth Agent system...[/bold green]"
            )
            ...
            if sample_data:
                ...
                if result["success"]:
                    console.print(
                        f"[green]✓ Installed {result['total_installed']} sample documents[/green]"
                    )
                    for store_type, count in result["installed_counts"].items():
                        if count > 0:
                            console.print(
                                f"  [dim]{store_type}: {count} documents[/dim]"
                            )
                else:
                    console.print(
                        f"[red]Error installing sample data: {result.get('error', 'Unknown error')}[/red]"
                    )
                    raise typer.Exit(1)
                ...
                if embed and result["total_installed"] > 0:
                    ...
                    async def embed_all_sample_data() -> int:
                        ...
                        for org_id in document_store.github_orgs.list():
                            try:
                                ...
                                console.print(
                                    f"[yellow]Warning: Could not embed org {org_id}: {e}[/yellow]"
                                )
                        ...
                        for repo_id in document_store.github_repos.list():
                            try:
                                ...
                                console.print(
                                    f"[yellow]Warning: Could not embed repo {repo_id}: {e}[/yellow]"
                                )
                        ...
                        for note_id in document_store.notes.list():
                            try:
                                ...
                                console.print(
                                    f"[yellow]Warning: Could not embed note {note_id}: {e}[/yellow]"
                                )
                    ...
                    try:
                        ...
                        console.print(
                            f"[green]✓ Embedded {embedded_count} documents[/green]"
                        )
                    except Exception as e:
                        console.print(
                            f"[yellow]Warning: Embedding failed: {e}[/yellow]"
                        )
            ...
            console.print(
                '[dim]  goldentooth-agent docs ask "What repositories are available?"[/dim]'
            )
...
def show_system_status() -> None:
    ...
    def handle(
        ...
        try:
            ...
            paths_table.add_row(
                "Data", str(data_dir), "✓" if data_dir.exists() else "✗"
            )
            paths_table.add_row(
