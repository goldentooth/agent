...
@app.command("index")
def index_codebase(
    codebase_name: Optional[str] = typer.Argument(None, help="Codebase name (default: goldentooth_agent)"),
    force: bool = typer.Option(False, "--force", "-f", help="Force full re-indexing (ignores change detection)"),
) -> None:
    """
    ...
    async def _index() -> None:
        ...
        if codebase_name:
            ...
            result = await introspection_service.collection.index_codebase(codebase_name, force_full_reindex=force)
        else:
            # Index current codebase
            if force:
                result = await introspection_service.collection.index_codebase("goldentooth_agent", force_full_reindex=True)
            else:
...
def query_codebase(
    ...
    doc_type: Optional[str] = typer.Option(None, "--type", "-t", help="Document type filter"),
    include_source: bool = typer.Option(True, "--source/--no-source", help="Include source code"),
    format_output: str = typer.Option("rich", "--format", "-f", help="Output format (rich, json)")
) -> None:
    """
    ...
    async def _query() -> None:
        ...
        if doc_type:
            try:
                ...
                console.print(f"Valid types: {', '.join([t.value for t in CodebaseDocumentType])}")
                return
...
def rag_query(
    ...
    codebase_weight: float = typer.Option(0.6, "--codebase-weight", "-w", help="Weight for codebase results (0-1)"),
    max_results: int = typer.Option(10, "--max-results", "-m", help="Maximum total results"),
    include_codebase: bool = typer.Option(True, "--codebase/--no-codebase", help="Include codebase search"),
    include_documents: bool = typer.Option(True, "--docs/--no-docs", help="Include document search")
) -> None:
    """
...
def codebase_overview(
    codebase_name: str = typer.Option("goldentooth_agent", "--codebase", "-c", help="Codebase name")
) -> None:
...
def compare_codebases(
    ...
    codebases: str = typer.Option("goldentooth_agent", "--codebases", "-c", help="Comma-separated codebase names")
) -> None:
    ...
    async def _compare() -> None:
        ...
        with console.status("🔍 Comparing codebases..."):
            comparison = await introspection_service.compare_codebases(query, codebase_list)
...
def add_codebase(
    ...
    display_name: Optional[str] = typer.Option(None, "--display-name", "-d", help="Human-readable name"),
    description: str = typer.Option("", "--description", help="Codebase description"),
    index_now: bool = typer.Option(True, "--index/--no-index", help="Index immediately after adding")
) -> None:
    ...
    async def _add() -> None:
        ...
        if index_now:
            ...
            console.print(f"📊 Documents: {result['documents_processed']}, Chunks: {result['chunks_created']}")
        else:
            console.print(f"✅ Added codebase '{name}' (not indexed)")
            console.print("💡 Run 'goldentooth-agent codebase index {name}' to index it")
...
def list_codebases() -> None:
    ...
    async def _list() -> None:
        ...
        if not codebases:
            console.print("No codebases available. Add one with 'goldentooth-agent codebase add'")
            return
...
def token_analysis(
    ...
    show_breakdown: bool = typer.Option(False, "--breakdown", "-b", help="Show detailed cost breakdown"),
    export_path: Optional[str] = typer.Option(None, "--export", "-e", help="Export data to file")
) -> None:
...
def budget_management(
    daily_limit: Optional[int] = typer.Option(None, "--daily-limit", help="Set daily token limit"),
    monthly_limit: Optional[int] = typer.Option(None, "--monthly-limit", help="Set monthly token limit"),
    warning_threshold: Optional[float] = typer.Option(None, "--warning-threshold", help="Warning threshold (0.0-1.0)"),
