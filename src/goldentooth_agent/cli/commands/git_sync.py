...
def setup_git_repository(
    repo_path: str = typer.Argument(..., help="Path where Git repository should be created"),
    remote_url: Optional[str] = typer.Option(None, "--remote", "-r", help="Remote repository URL"),
):
    ...
    def handle(git_sync: GitDataSync = inject.me()) -> None:
        ...
        try:
            ...
            with console.status(f"[bold green]Setting up Git repository at {repo_path_obj}..."):
        ...
        try:
            ...
            if result["success"]:
                ...
                if "files_changed" in result and result["files_changed"]:
                    console.print(f"[cyan]Files committed: {len(result['files_changed'])}[/cyan]")
                    console.print(f"[dim]Commit message: {result['commit_message']}[/dim]")
                    
                    if push:
                        if result.get("pushed"):
                            console.print("[green]✓ Pushed to remote[/green]")
                        else:
                            console.print("[yellow]⚠️  Could not push to remote[/yellow]")
                            if result.get("push_error"):
                                console.print(f"[dim]Error: {result['push_error']}[/dim]")
                else:
    ...
    def handle(git_sync: GitDataSync = inject.me()) -> None:
        ...
        try:
            ...
            if not status["exists"]:
                ...
                console.print("[dim]Run 'goldentooth-agent git setup <path>' to create one[/dim]")
                raise typer.Exit(1)
            ...
            table.add_row("Current Branch", status["current_branch"] or "[dim]detached[/dim]")
            ...
            if status["last_commit_hash"]:
                ...
                table.add_row("Commit Message", status["last_commit_message"] or "[dim]none[/dim]")
                table.add_row("Commit Date", status["last_commit_date"] or "[dim]none[/dim]")
            else:
...
def auto_sync_after_github_sync(
    repo_path: str = typer.Argument(..., help="Path to Git repository"),
    org_name: Optional[str] = typer.Option(None, "--org", help="GitHub organization to sync first"),
    ...
    def handle(git_sync: GitDataSync = inject.me()) -> None:
        ...
        try:
            ...
            if org_name:
                console.print(f"[cyan]Step 1: Syncing GitHub organization '{org_name}'...[/cyan]")
                # Import here to avoid circular imports
                from ...core.github import GitHubClient
                from antidote import world
                
                github_client = world.get(GitHubClient)
                github_result = github_client.sync_organization(org_name, include_repos=True)
                
                if github_result.get("error"):
                    console.print(f"[red]GitHub sync failed: {github_result['error']}[/red]")
                    raise typer.Exit(1)
                
                console.print(f"[green]✓ Synced {github_result['repos_synced']} repositories[/green]")
            
            # Then sync to Git
            console.print("[cyan]Step 2: Syncing to Git repository...[/cyan]")
            
            commit_message = f"Auto-sync: GitHub data update"
            if org_name:
            ...
            if git_result["success"]:
                ...
                console.print(f"[cyan]Files copied: {git_result['files_copied']}[/cyan]")
                
                if git_result.get("files_changed"):
                    console.print(f"[cyan]Files committed: {len(git_result['files_changed'])}[/cyan]")
                ...
                console.print(f"[dim]GitHub → Local Storage → Git Repository{' → Remote' if push else ''}[/dim]")
            else:
