...
async def demo_agent_interaction(console: Console) -> None:
    ...
    for i, message in enumerate(messages, 1):
        ...
        async for result in agent_flow(input_stream()):
            console.print(f"[green]Response:[/green] {result.response}")  # type: ignore[attr-defined]  # AgentOutput has response
            if result.metadata:  # type: ignore[attr-defined]  # AgentOutput has metadata
                metadata_str = " | ".join(
