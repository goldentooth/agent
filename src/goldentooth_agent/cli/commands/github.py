...
def sync_organization(
    ...
    embed: bool = typer.Option(True, "--embed/--no-embed", help="Embed documents after sync"),
):
    ...
    def handle(
        ...
        try:
            ...
            if result.get("error"):
                console.print(f"[red]Error syncing organization: {result['error']}[/red]")
                raise typer.Exit(1)
            
            # Show sync results
            console.print(f"[green]✓ Organization '{org_name}' synced successfully[/green]")
            if repos:
                console.print(f"[cyan]  Synced {result['repos_synced']} repositories[/cyan]")
                ...
                async def embed_docs():
                    ...
                    if repos:
                        ...
                        for repo_id in repo_ids:
                            try:
                                repo_doc = github_client.document_store.github_repos.load(repo_id)
                                ...
                                console.print(f"[yellow]Warning: Could not embed {repo_id}: {e}[/yellow]")
                    
                    return embedded_repos
                
                try:
                    embedded_repos = asyncio.run(embed_docs())
                    console.print(f"[green]✓ Embedded organization + {embedded_repos} repositories[/green]")
                except Exception as e:
...
def sync_user_repos(
    username: Optional[str] = typer.Option(None, "--user", "-u", help="Username (default: authenticated user)"),
    max_repos: Optional[int] = typer.Option(20, "--max", "-m", help="Maximum repositories to sync"),
    embed: bool = typer.Option(True, "--embed/--no-embed", help="Embed documents after sync"),
):
    ...
    def handle(
        ...
        try:
            ...
            with console.status(f"[bold green]Syncing repositories for {user_display}..."):
                result = github_client.sync_user_repos(username=username, max_repos=max_repos)
            
            if result.get("error"):
                console.print(f"[red]Error syncing user repositories: {result['error']}[/red]")
                raise typer.Exit(1)
            
            # Show sync results
            console.print(f"[green]✓ Synced {result['repos_synced']} repositories for {user_display}[/green]")
            ...
            if embed and result['repos_synced'] > 0:
                    ...
                    for repo_id in repo_ids:
                        try:
                            ...
                            if repo_doc.last_synced and (
                                repo_doc.last_synced.isoformat() == result["timestamp"] or
                                abs((repo_doc.last_synced.timestamp() - 
                                    datetime.fromisoformat(result["timestamp"]).timestamp())) < 60
                            ):
                                repo_embedding = await embeddings_service.create_document_embedding(
                                    repo_doc.model_dump()
                                )
                ...
                try:
                    ...
                    console.print(f"[green]✓ Embedded {embedded_count} repositories[/green]")
                except Exception as e:
                    console.print(f"[yellow]Warning: Embedding failed: {e}[/yellow]")
...
def list_user_organizations():
    ...
    def handle(github_client: GitHubClient = inject.me()) -> None:
        ...
        try:
            ...
            if not orgs:
                console.print("[yellow]No organizations found for the authenticated user.[/yellow]")
                return
    ...
    def handle(github_client: GitHubClient = inject.me()) -> None:
        ...
        try:
            ...
            if rate_limit.get("error"):
                console.print(f"[red]Error getting rate limit: {rate_limit['error']}[/red]")
                raise typer.Exit(1)
            ...
            core_reset = datetime.fromtimestamp(rate_limit["core_reset"]).strftime("%H:%M:%S")
            search_reset = datetime.fromtimestamp(rate_limit["search_reset"]).strftime("%H:%M:%S")
            
            table.add_row("Core", str(rate_limit["core_limit"]), str(rate_limit["core_remaining"]), core_reset)
            table.add_row("Search", str(rate_limit["search_limit"]), str(rate_limit["search_remaining"]), search_reset)
            
            console.print(table)
            
            # Warning if running low
            if rate_limit["core_remaining"] < 100:
                console.print("[yellow]⚠️  Warning: Core API rate limit is running low[/yellow]")
    ...
    def handle(document_store: DocumentStore = inject.me()) -> None:
        ...
        try:
            ...
            if len(document_store.github_orgs.list()) > 3:
                org_examples += f", ... (+{len(document_store.github_orgs.list()) - 3} more)"
            
            table.add_row("Organizations", str(org_count), org_examples or "[dim]none[/dim]")
            
            # Repositories
            repo_examples = ", ".join(document_store.github_repos.list()[:3])
            if len(document_store.github_repos.list()) > 3:
                repo_examples += f", ... (+{len(document_store.github_repos.list()) - 3} more)"
            
            table.add_row("Repositories", str(repo_count), repo_examples or "[dim]none[/dim]")
