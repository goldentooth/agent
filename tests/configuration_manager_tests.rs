use goldentooth_agent::config::ConfigurationManager;
...
"#;
    ...
    let config = config_manager.unwrap().get_config().unwrap();
...
async fn configuration_manager_can_save_to_file() {
    ...
    let config_manager = ConfigurationManager::new();
    config_manager
        .set_cluster_endpoint("https://test.goldentooth.net")
        .unwrap();
...
async fn configuration_manager_can_get_config_value_by_key() {
    let config_manager = ConfigurationManager::new();
    config_manager
        .set_cluster_endpoint("https://test.example.com")
        .unwrap();
