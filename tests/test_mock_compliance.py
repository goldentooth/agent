...
class TestMockCompliance:
    ...
    def test_vector_store_mock_has_all_public_methods(self):
        ...
        real_methods = {
            name for name, method in inspect.getmembers(VectorStore, inspect.isfunction)
            if not name.startswith('_')
        }
        
        mock_methods = {
            name for name, method in inspect.getmembers(TypeSafeMockVectorStore, inspect.isfunction)
            if not name.startswith('_')
        }
        
        # These are the methods we care about mocking
        critical_methods = {
            'store_document', 'store_document_chunks', 'search_similar', 
            'get_document', 'delete_document', 'get_stats'
        }
    ...
    def test_store_document_signature_matches(self):
        ...
        assert real_params == mock_params, f"Parameter mismatch: {real_params} vs {mock_params}"
        
        # Check return types if available
        real_return = real_sig.return_annotation
        mock_return = mock_sig.return_annotation
        
        if real_return != inspect.Signature.empty and mock_return != inspect.Signature.empty:
            assert real_return == mock_return, f"Return type mismatch: {real_return} vs {mock_return}"

    def test_all_protocol_methods_have_implementations(self):
        """Test that all protocol methods are implemented in the mock."""
        protocol_methods = {
            name for name in dir(VectorStoreProtocol)
            if not name.startswith('_') and callable(getattr(VectorStoreProtocol, name, None))
        }
        
        mock_methods = {
            name for name in dir(TypeSafeMockVectorStore)
            if not name.startswith('_') and callable(getattr(TypeSafeMockVectorStore, name))
        }
    ...
    @pytest.mark.parametrize("method_name", [
        "store_document", "store_document_chunks", "search", 
        "get_document", "delete_document", "get_stats"
    ])
    def test_method_signatures_are_compatible(self, method_name: str):
        ...
        if hasattr(VectorStore, method_name):
            ...
            assert len(real_sig.parameters) == len(mock_sig.parameters), \
