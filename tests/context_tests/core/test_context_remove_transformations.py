...
class TestContextRemoveTransformations:
    ...
    def test_remove_transformations_basic(self) -> None:
        ...
        # Verify transformations were removed
        assert "name" not in context._transformations
    ...
    def test_remove_transformations_multiple_keys(self) -> None:
        ...
        # Verify only key2 transformations were removed
        assert "key1" in context._transformations
        assert "key2" not in context._transformations
    ...
    def test_remove_transformations_multiple_per_key(self) -> None:
        ...
        assert "value" not in context._transformations
    ...
    def test_remove_transformations_after_key_exists(self) -> None:
        ...
        assert "temp" not in context._transformations
    ...
    def test_remove_transformations_partial_removal(self) -> None:
        ...
        assert len(context._transformations) == 3

        # Remove transformations for "remove" key
        context.remove_transformations("remove")

        # Verify selective removal
        assert "remove" not in context._transformations
        assert "keep1" in context._transformations
        assert "keep2" in context._transformations
    ...
    def test_remove_transformations_special_characters_key(self) -> None:
        ...
        assert "key.with.dots" not in context._transformations
        assert "key-with-dashes" in context._transformations
        assert "key_with_underscores" in context._transformations
    ...
    def test_remove_transformations_empty_key(self) -> None:
        ...
        # Verify removal
        assert "" not in context._transformations
    ...
    def test_remove_transformations_independence(self) -> None:
        ...
        # Verify independence
        assert "shared" not in context1._transformations
        assert "shared" in context2._transformations
        assert len(context2._transformations["shared"]) == 1
    ...
    def test_remove_transformations_sequence(self) -> None:
        ...
        assert len(context._transformations["key"]) == 2

        # Remove all transformations
        context.remove_transformations("key")
        assert "key" not in context._transformations
    ...
    def test_remove_transformations_mixed_operations(self) -> None:
        ...
        # Verify final state
        assert "remove_later" not in context._transformations
        assert "keep" in context._transformations
        assert len(context._transformations["keep"]) == 3
