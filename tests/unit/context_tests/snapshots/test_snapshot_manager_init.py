...
class TestSnapshotManagerInit:
    ...
    def test_init_multiple_instances_isolated(self) -> None:
        ...
        assert manager1._snapshots is not manager2._snapshots

        # Modifying one should not affect the other
        from unittest.mock import Mock

        mock_snapshot = Mock()
        manager1._snapshots["test"] = mock_snapshot
        assert "test" not in manager2._snapshots
    ...
    def test_init_attributes_types(self) -> None:
        ...
        mock_snapshot = Mock()
        manager._snapshots["test_name"] = mock_snapshot
        assert "test_name" in manager._snapshots
    ...
    def test_init_no_parameters_required(self) -> None:
        ...
        with pytest.raises(TypeError):
            SnapshotManager("extra_arg")  # type: ignore[call-arg]
    ...
    def test_init_callable_immediately(self) -> None:
        ...
        assert "nonexistent" not in manager._snapshots
    ...
    def test_init_memory_efficiency(self) -> None:
        ...
        assert type(manager._snapshots) is dict
