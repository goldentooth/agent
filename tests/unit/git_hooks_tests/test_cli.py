...
# Test utility functions (from conftest.py)
...
class TestCLI:
    ...
    def test_file_length_hook_passes_small_files(self, temp_git_repo: Path) -> None:
        ...
        with patch("subprocess.run") as mock_run:
            ...
            with patch("git_hooks.utils.get_staged_files", return_value=[small_file]):
    ...
    def test_file_length_hook_fails_large_files(self, temp_git_repo: Path) -> None:
        ...
        with patch("git_hooks.hook_runner.is_git_repo", return_value=True):
            with patch(
                "git_hooks.hook_runner.get_staged_files", return_value=[large_file]
            ):
                result = cli.check_file_length()
    ...
    def test_module_size_hook_passes_small_modules(self, temp_git_repo: Path) -> None:
        ...
        with patch(
            "git_hooks.hook_runner.get_modules",
    ...
    def test_hooks_work_outside_git_repo(self, tmp_path: Path) -> None:
        ...
        with patch("git_hooks.hook_runner.is_git_repo", return_value=False):
            with patch(
                "git_hooks.hook_runner.get_all_files", return_value=[large_file]
            ):
                result = cli.check_file_length()
    ...
    def test_module_size_warnings_success(self, temp_git_repo: Path) -> None:
        ...
        with patch("git_hooks.utils.get_modules", return_value=[large_module]):
    ...
    def test_file_length_no_files_to_check(
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(returncode=0, stdout="", stderr="")
            with patch("git_hooks.utils.get_staged_files", return_value=[]):
                result = cli.check_file_length()
    ...
    def test_module_size_no_modules(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        with patch("git_hooks.hook_runner.get_modules", return_value=[]):
            result = cli.check_module_size()
    ...
    def test_file_length_warnings_outside_git_repo(
        ...
        with patch("subprocess.run") as mock_run:
            ...
            with patch("git_hooks.utils.get_all_files", return_value=[warning_file]):
                result = cli.check_file_length_warnings()
    ...
    def test_get_all_files_default_directory(self, tmp_path: Path) -> None:
        ...
        with patch("git_hooks.utils.Path") as mock_path:
    ...
    def test_get_files_by_glob_git_context_staged_files(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            with patch("git_hooks.cli.is_git_repo", return_value=True):
                with patch(
                    "git_hooks.cli.get_staged_files", return_value=[staged_file]
                ):
    ...
    def test_get_files_by_glob_all_files_ignores_git(self, tmp_path: Path) -> None:
        ...
        try:
            os.chdir(tmp_path)
            with patch("git_hooks.cli.is_git_repo", return_value=True):
                with patch("git_hooks.cli.get_staged_files", return_value=[file1]):
    ...
    def test_file_length_command_warnings_mode(self, tmp_path: Path) -> None:
        ...
        with patch("git_hooks.cli._run_validation") as mock_run:
    ...
    def test_run_validation_with_glob_files_found(self, tmp_path: Path) -> None:
        ...
        with patch("git_hooks.cli.get_files_by_glob", return_value=[test_file]):
            with patch(
                "git_hooks.cli.run_hook_with_config", return_value=0
            ) as mock_run:
                try:
    ...
    def test_run_validation_with_glob_no_files_found(self) -> None:
        ...
        with patch("git_hooks.cli.get_files_by_glob", return_value=[]):
    ...
    def test_run_validation_with_all_files(self) -> None:
        ...
        with patch("git_hooks.cli.get_all_files", return_value=test_files):
            with patch(
                "git_hooks.cli.run_hook_with_config", return_value=0
            ) as mock_run:
                try:
    ...
    def test_run_validation_default_behavior(self) -> None:
        ...
        with patch("git_hooks.cli.run_hook_with_config", return_value=1) as mock_run:
            try:
