...
from flow.combinators.temporal import (
    DebounceMode,
    debounce_stream,
...
from flow.exceptions import FlowTimeoutError
...
class TestSampleAdvanced:
    ...
    async def test_sample_with_transform(self) -> None:
        ...
        from flow.combinators.basic import compose, map_stream
        from flow.combinators.temporal import sample_stream
...
class TestTemporalErrorHandling:
    ...
    async def test_timeout_stream_zero_timeout(self) -> None:
        ...
        with pytest.raises(FlowTimeoutError):
            _ = [item async for item in timeout_flow(simple_stream())]
