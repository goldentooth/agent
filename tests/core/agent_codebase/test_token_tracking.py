...
class TestTokenTracker:
    ...
    def test_budget_reset_logic(self, token_tracker: TokenTracker):
        ...
        with patch(
            "goldentooth_agent.core.agent_codebase.token_tracking.datetime"
        ) as mock_datetime:
    ...
    def test_database_initialization(self, temp_dir: Path):
        ...
        with sqlite3.connect(db_file) as conn:
            ...
            cursor.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='token_budget'"
            )
            assert cursor.fetchone() is not None
    ...
    def test_database_record_storage(self, token_tracker: TokenTracker):
        ...
        with sqlite3.connect(token_tracker.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM token_usage WHERE operation_id = ?",
                (record.operation_id,),
            )
            row = cursor.fetchone()
