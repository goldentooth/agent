...
class TestCodebaseIntrospectionService:
    ...
    @pytest.mark.asyncio
    async def test_add_external_codebase(
        self, introspection_service, sample_codebase_files: Path
    ):
    ...
    async def test_compare_codebases(self, introspection_service):
        ...
        comparison = await introspection_service.compare_codebases(
            query="error handling patterns", codebase_names=["goldentooth_agent"]
        )
    ...
    async def test_get_codebase_overview(self, introspection_service):
        ...
        overview = await introspection_service.get_codebase_overview(
            "goldentooth_agent"
        )
    ...
    def test_infer_document_types(self, introspection_service):
        ...
        types = introspection_service._infer_document_types(
            "show me the function definition"
        )
        ...
        types = introspection_service._infer_document_types(
            "explain the system design and motivation"
        )
    ...
    def test_list_available_codebases(self, introspection_service):
        ...
        info1 = CodebaseInfo(
            name="test1", display_name="Test 1", root_path=Path("/test1")
        )
        info2 = CodebaseInfo(
            name="test2", display_name="Test 2", root_path=Path("/test2")
        )
...
class TestCodebaseRAGIntegration:
    ...
    def mock_rag_service(self):
        ...
        mock.claude_client.generate_response = AsyncMock(
            return_value={"content": "Synthesized answer from RAG and codebase data"}
        )
        return mock
    ...
    async def test_combined_query(self, rag_integration, mock_introspection_service):
        ...
        from goldentooth_agent.core.agent_codebase.introspection import (
            IntrospectionResult,
        )
    ...
    @pytest.mark.asyncio
    async def test_codebase_only_query(
        self, rag_integration, mock_introspection_service
    ):
    ...
    def test_combine_results(self, rag_integration):
        ...
        # Check weighting
        codebase_item = next(
            item for item in combined if item["source_type"] == "codebase"
        )
        document_item = next(
            item for item in combined if item["source_type"] == "documents"
        )
    ...
    async def test_synthesis_with_mixed_content(self, rag_integration):
        ...
        answer = await rag_integration._synthesize_answer(
            "how to authenticate", results
        )
    ...
    @pytest.mark.asyncio
    async def test_get_codebase_overview_integration(
        self, rag_integration, mock_introspection_service
    ):
        """Test getting codebase overview through integration."""
        mock_introspection_service.get_codebase_overview = AsyncMock(
            return_value={"overview": "test overview"}
        )

        overview = await rag_integration.get_codebase_overview()

        assert overview["overview"] == "test overview"
        mock_introspection_service.get_codebase_overview.assert_called_once_with(
            "goldentooth_agent"
        )

    @pytest.mark.asyncio
    async def test_index_codebase_for_rag(
        self, rag_integration, mock_introspection_service
    ):
        """Test indexing codebase for RAG integration."""
        mock_introspection_service.index_current_codebase = AsyncMock(
...
class TestEndToEndWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_full_indexing_and_query_workflow(
        self, mock_vector_store, sample_codebase_files: Path
    ):
        ...
        search_results = await collection.search(
            query="function", codebase_names=["test_workflow"], limit=5
        )
