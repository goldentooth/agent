...
class TestCodebaseCollection:
    ...
    @pytest.mark.asyncio
    async def test_index_codebase_new(
    ...
    @pytest.mark.asyncio
    async def test_index_codebase_incremental(
        self, mock_vector_store, sample_codebase_files: Path
    ):
        ...
        await collection.add_codebase(
            name="test_codebase", root_path=sample_codebase_files
        )
    ...
    @pytest.mark.asyncio
    async def test_index_codebase_force_reindex(
        self, mock_vector_store, sample_codebase_files: Path
    ):
        ...
        # Force reindex
        result2 = await collection.index_codebase(
            "test_codebase", force_full_reindex=True
        )
    ...
    def test_chunk_document_large_content(self, mock_vector_store):
        ...
        for chunk in chunks:
            assert (
                len(chunk) <= large_doc.get_chunk_size_hint() * 1.2
            )  # Allow some overflow
...
Fourth paragraph wrapping up the content."""
    ...
    def test_get_codebase_info(self, mock_vector_store):
        ...
        info = CodebaseInfo(
            name="test_codebase", display_name="Test", root_path=Path("/test")
        )
    ...
    def test_list_codebases(self, mock_vector_store):
        ...
        info1 = CodebaseInfo(
            name="codebase1", display_name="First", root_path=Path("/first")
        )
        info2 = CodebaseInfo(
            name="codebase2", display_name="Second", root_path=Path("/second")
        )
    ...
    @pytest.mark.asyncio
    async def test_error_handling_in_extraction(
        self, mock_vector_store, temp_dir: Path
    ):
        ...
        await collection.add_codebase(name="error_codebase", root_path=test_dir)
