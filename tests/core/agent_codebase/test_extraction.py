...
class TestCodebaseDocumentExtractor:
    ...
    @pytest.mark.asyncio
    async def test_extract_from_python_file(
        self, document_extractor: CodebaseDocumentExtractor, sample_codebase_files: Path
    ):
        """Test extracting documents from Python files."""
        main_py = sample_codebase_files / "src" / "test_package" / "main.py"

        documents = await document_extractor._extract_from_file(
            main_py, sample_codebase_files
        )
        ...
        function_docs = [
            d
            for d in documents
            if d.document_type == CodebaseDocumentType.FUNCTION_DEFINITION
        ]
        class_docs = [
            d
            for d in documents
            if d.document_type == CodebaseDocumentType.CLASS_DEFINITION
        ]

        # Check function extraction
        assert len(function_docs) >= 1
        func_doc = next(
            (d for d in function_docs if "utility_function" in d.title), None
        )
        assert func_doc is not None
        assert (
            "def utility_function(x, y)" in func_doc.signature
        )  # Type hints stripped in signature
    ...
    @pytest.mark.asyncio
    async def test_extract_from_markdown_file(
        self, document_extractor: CodebaseDocumentExtractor, sample_codebase_files: Path
    ):
        ...
        documents = await document_extractor._extract_from_file(
            readme_md, sample_codebase_files
        )
    ...
    @pytest.mark.asyncio
    async def test_extract_from_background_file(
        self, document_extractor: CodebaseDocumentExtractor, sample_codebase_files: Path
    ):
    ...
    @pytest.mark.asyncio
    async def test_extract_from_path(
        self, document_extractor: CodebaseDocumentExtractor, sample_codebase_files: Path
    ):
        ...
        expected_modules = {
            "test_package",
            "test_package.utils",
            "test_package.submodule.worker",
        }
        assert expected_modules.issubset(module_paths)

    def test_get_module_path(
        self, document_extractor: CodebaseDocumentExtractor, temp_dir: Path
    ):
    ...
    def test_walk_files(
        ...
        python_files = document_extractor._walk_files(
            src_path, include_patterns=["*.py"], exclude_patterns=[]
        )
        ...
        md_files = document_extractor._walk_files(
            src_path, include_patterns=["*.md"], exclude_patterns=[]
        )
        ...
        non_init_files = document_extractor._walk_files(
            src_path, include_patterns=["*.py"], exclude_patterns=["__init__.py"]
        )
...
"""
    ...
    def test_get_function_signature(
        self, document_extractor: CodebaseDocumentExtractor
    ):
...
"""
    ...
    def test_markdown_summary_extraction(
        self, document_extractor: CodebaseDocumentExtractor
    ):
...
"""
    ...
    @pytest.mark.asyncio
    async def test_file_permissions_error(
        self, document_extractor: CodebaseDocumentExtractor, temp_dir: Path
    ):
