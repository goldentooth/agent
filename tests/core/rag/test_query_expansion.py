...
class TestQueryExpansionEngine:
    ...
    def test_detect_intent_factual(
        ...
        assert (
            expansion_engine._detect_intent("what does docker do")
            == QueryIntent.FACTUAL
        )
        assert (
            expansion_engine._detect_intent("when is the best time")
            == QueryIntent.FACTUAL
        )

    def test_detect_intent_procedural(
        self, expansion_engine: QueryExpansionEngine
    ) -> None:
        """Test intent detection for procedural queries."""
        assert (
            expansion_engine._detect_intent("how to install python")
            == QueryIntent.PROCEDURAL
        )
        assert (
            expansion_engine._detect_intent("steps to deploy docker")
            == QueryIntent.PROCEDURAL
        )
        assert (
            expansion_engine._detect_intent("guide to kubernetes setup")
            == QueryIntent.PROCEDURAL
        )

    def test_detect_intent_troubleshooting(
        self, expansion_engine: QueryExpansionEngine
    ) -> None:
        """Test intent detection for troubleshooting queries."""
        assert (
            expansion_engine._detect_intent("python error import module")
            == QueryIntent.TROUBLESHOOTING
        )
        assert (
            expansion_engine._detect_intent("fix docker not working")
            == QueryIntent.TROUBLESHOOTING
        )
        assert (
            expansion_engine._detect_intent("debug kubernetes pod failure")
            == QueryIntent.TROUBLESHOOTING
        )

    def test_detect_intent_comparative(
        self, expansion_engine: QueryExpansionEngine
    ) -> None:
        """Test intent detection for comparative queries."""
        assert (
            expansion_engine._detect_intent("python vs java") == QueryIntent.COMPARATIVE
        )
        assert (
            expansion_engine._detect_intent("difference between docker and kubernetes")
            == QueryIntent.COMPARATIVE
        )
        assert (
            expansion_engine._detect_intent("flask compared to django")
            == QueryIntent.COMPARATIVE
        )
    ...
    def test_detect_intent_general(
        ...
        assert (
            expansion_engine._detect_intent("python programming") == QueryIntent.GENERAL
        )
        assert (
            expansion_engine._detect_intent("machine learning") == QueryIntent.GENERAL
        )

    def test_extract_key_terms(self, expansion_engine: QueryExpansionEngine) -> None:
        """Test key term extraction."""
        terms = expansion_engine._extract_key_terms(
    ...
    def test_generate_synonyms_with_context(
        self, expansion_engine: QueryExpansionEngine
    ) -> None:
        """Test synonym generation with domain context."""
        synonyms = expansion_engine._generate_synonyms(
            ["python"], domain_context="python"
        )
    ...
    def test_generate_related_terms(
        self, expansion_engine: QueryExpansionEngine
    ) -> None:
        ...
        assert (
            "configuration" in related
            or "documentation" in related
            or "tutorial" in related
        )

    def test_generate_term_variations(
        self, expansion_engine: QueryExpansionEngine
    ) -> None:
    ...
    def test_expand_query_with_domain_context(
        self, expansion_engine: QueryExpansionEngine
    ) -> None:
    ...
    def test_expand_query_confidence_calculation(
        self, expansion_engine: QueryExpansionEngine
    ) -> None:
    ...
    def test_create_search_strategies(
        self, expansion_engine: QueryExpansionEngine
    ) -> None:
        """Test search strategy creation."""
        expansion = expansion_engine.expand_query("python programming tutorial")
        strategies = expansion_engine.create_search_strategies(
            expansion, max_strategies=3
        )
    ...
    def test_create_search_strategies_with_synonyms(
        self, expansion_engine: QueryExpansionEngine
    ) -> None:
    ...
    def test_analyze_query_quality(
        self, expansion_engine: QueryExpansionEngine
    ) -> None:
        """Test query quality analysis."""
        # Good quality query
        analysis1 = expansion_engine.analyze_query_quality(
    ...
    def test_quality_metrics(self, expansion_engine: QueryExpansionEngine) -> None:
        ...
        # Test length scoring
        assert (
            expansion_engine._score_query_length("python programming guide") == 1.0
        )  # Optimal
        assert expansion_engine._score_query_length("python") < 1.0  # Too short
        assert (
            expansion_engine._score_query_length(
                "a very long query with many words that goes on"
            )
            < 1.0
        )  # Too long
    ...
    def test_reformulate_query_no_results(
        self, expansion_engine: QueryExpansionEngine
    ) -> None:
    ...
    def test_reformulate_query_poor_quality(
        self, expansion_engine: QueryExpansionEngine
    ) -> None:
    ...
    def test_reformulate_query_too_many_results(
        self, expansion_engine: QueryExpansionEngine
    ) -> None:
    ...
    def test_intent_specific_expansions(
        self, expansion_engine: QueryExpansionEngine
    ) -> None:
        ...
        troubleshooting_expansions = (
            expansion_engine._create_intent_specific_expansions(
    ...
    def test_get_intent_search_params(
        self, expansion_engine: QueryExpansionEngine
    ) -> None:
        ...
        # Procedural queries
        procedural_params = expansion_engine._get_intent_search_params(
            QueryIntent.PROCEDURAL
        )
    ...
    def test_create_more_specific_queries(
        self, expansion_engine: QueryExpansionEngine
    ) -> None:
    ...
    def test_domain_related_terms(self, expansion_engine: QueryExpansionEngine) -> None:
        ...
        # Python domain
        python_related = expansion_engine._get_domain_related_terms(
            "function", "python"
        )
        ...
        # GitHub domain
        github_related = expansion_engine._get_domain_related_terms(
            "repository", "github"
        )
        ...
        unknown_related = expansion_engine._get_domain_related_terms(
            "unknown", "unknown"
        )
    ...
    def test_comprehensive_integration(
        ...
        assert any(
            term in ["python", "import", "error", "django"]
