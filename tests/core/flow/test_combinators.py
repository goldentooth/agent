...
class TestCombinatorComposition:
    ...
    async def test_complex_pipeline(self):
        ...
        pipeline = compose(
            compose(
                compose(filter_stream(is_even), map_stream(increment)), batch_stream(2)
            ),
            take_stream(1),
        )
    ...
    async def test_tap_in_pipeline(self):
        ...
        pipeline = compose(
            compose(map_stream(increment), tap_stream(side_effect)), map_stream(double)
        )
...
class TestNewCoreCombinators:
    """Test cases for newly added core combinators."""
    ...
    @pytest.mark.asyncio
    async def test_flat_map_ctx_stream_basic(self):
        """Test basic flat_map_ctx_stream functionality."""

        async def ctx_aware_expand(item, original):
            """Expand item based on its relationship to original context."""
            for i in range(item + 1):
                yield f"{item}_{i}_ctx_{original}"

        flat_map_ctx = flat_map_ctx_stream(ctx_aware_expand)
        assert "flat_map_ctx" in flat_map_ctx.name

        input_stream = async_range(3)  # [0, 1, 2]
        result_stream = flat_map_ctx(input_stream)
        values = [item async for item in result_stream]

        # Each item creates expansions with its original context
        expected = [
            "0_0_ctx_0",  # item 0: range(0+1) = [0]
            "1_0_ctx_1",
            "1_1_ctx_1",  # item 1: range(1+1) = [0,1]
            "2_0_ctx_2",
            "2_1_ctx_2",
            "2_2_ctx_2",  # item 2: range(2+1) = [0,1,2]
        ]
        assert values == expected

    @pytest.mark.asyncio
    async def test_flat_map_ctx_stream_empty(self):
        """Test flat_map_ctx_stream with empty stream."""

        async def ctx_expand(item, original):
            yield f"{item}_{original}"

        flat_map_ctx = flat_map_ctx_stream(ctx_expand)
    ...
    @pytest.mark.asyncio
    async def test_guard_stream_success(self):
        """Test guard_stream when all items pass the predicate."""
        guard_positive = guard_stream(is_positive, "Must be positive")
        assert guard_positive.name == "guard(is_positive)"

        # Create a stream with positive numbers only
        async def positive_stream():
            for i in [1, 2, 3]:
                yield i

        input_stream = positive_stream()
        result_stream = guard_positive(input_stream)
        values = [item async for item in result_stream]
        assert values == [1, 2, 3]

    @pytest.mark.asyncio
    async def test_guard_stream_failure(self):
        """Test guard_stream when an item fails the predicate."""
        guard_positive = guard_stream(is_positive, "Must be positive")

        input_stream = async_range(4)  # [0, 1, 2, 3] - 0 will fail
        result_stream = guard_positive(input_stream)
        ...
        # Should not process any items after the guard failure
        assert values == []

    @pytest.mark.asyncio
    async def test_guard_stream_custom_message(self):
        """Test guard_stream with custom error message."""
        guard_even = guard_stream(is_even, "Numbers must be even")

        input_stream = async_range(3)  # [0, 1, 2] - 1 will fail
        ...
        # Should process first item (0) before failing on second (1)
        assert values == [0]

    @pytest.mark.asyncio
    async def test_then_stream_basic(self):
        """Test basic then_stream functionality."""
        then_side_effect = then_stream(side_effect)
        assert "then" in then_side_effect.name

        input_stream = async_range(3)
        result_stream = then_side_effect(input_stream)
        values = [item async for item in result_stream]
    ...
    @pytest.mark.asyncio
    async def test_then_stream_sync_function(self):
        """Test then_stream with synchronous side effect function."""

        def sync_side_effect(x):
            side_effects.append(f"sync: {x}")

        then_sync = then_stream(sync_side_effect)

        input_stream = async_range(2)
        result_stream = then_sync(input_stream)
        values = [item async for item in result_stream]

        assert values == [0, 1]
        assert side_effects == ["sync: 0", "sync: 1"]
    ...
    async def test_memoize_stream_basic(self):
        ...
        async def dup_stream():
            for val in [1, 2, 1, 3, 2, 1]:
                yield val

        input_stream = dup_stream()
        result_stream = memo(input_stream)
        values = [item async for item in result_stream]

        # All values should pass through (memoize doesn't change values, just caches)
        assert values == [1, 2, 1, 3, 2, 1]

    @pytest.mark.asyncio
    async def test_memoize_stream_custom_key(self):
        """Test memoize_stream with custom key function."""
        ...
        # Should see: 1 (odd), 3->1 (cached odd), 2 (even), 4->2 (cached even), 5->1 (cached odd)
        assert values == [1, 1, 2, 2, 1]

    @pytest.mark.asyncio
    async def test_while_condition_stream_basic(self):
        """Test while_condition_stream basic functionality."""
        double_while_small = while_condition_stream(
            lambda x: x < 3, map_stream(lambda x: x * 2)
        )

        assert "while" in double_while_small.name

        input_stream = async_range(5)  # [0, 1, 2, 3, 4]
        result_stream = double_while_small(input_stream)
        values = [item async for item in result_stream]

        # Process 0,1,2 (doubled), then 3,4 pass through unchanged
        assert values == [0, 2, 4, 3, 4]

    @pytest.mark.asyncio
    async def test_while_condition_stream_early_exit(self):
        """Test while_condition_stream stops at first false condition."""
        increment_while_even = while_condition_stream(is_even, map_stream(increment))

        input_stream = async_range(4)  # [0, 1, 2, 3]
        result_stream = increment_while_even(input_stream)
        values = [item async for item in result_stream]

        # 0 is even -> incremented to 1, then 1 is odd -> stops, passes 1,2,3 unchanged
        assert values == [1, 1, 2, 3]

    @pytest.mark.asyncio
    async def test_while_condition_stream_all_true(self):
        """Test while_condition_stream when condition is always true."""
        increment_while_positive = while_condition_stream(
            lambda x: x >= 0, map_stream(increment)  # Always true for our range
        )

        input_stream = async_range(3)
        result_stream = increment_while_positive(input_stream)
        values = [item async for item in result_stream]
    ...
    @pytest.mark.asyncio
    async def test_flatten_stream_basic(self):
        """Test basic flatten_stream functionality."""

        async def create_nested_streams():
        ...
        flatten = flatten_stream()
        assert flatten.name == "flatten"

        nested_stream = create_nested_streams()
        result_stream = flatten(nested_stream)
        values = [item async for item in result_stream]

        # Should flatten all nested streams into one
        assert values == [1, 2, 3, 4, 5]

    @pytest.mark.asyncio
    async def test_flatten_stream_empty(self):
        """Test flatten_stream with empty outer stream."""

        async def empty_nested():
            return  # No nested streams
            yield  # unreachable

        flatten = flatten_stream()
        nested_stream = empty_nested()
        result_stream = flatten(nested_stream)
        values = [item async for item in result_stream]
    ...
    @pytest.mark.asyncio
    async def test_flatten_stream_single_nested(self):
        """Test flatten_stream with single nested stream."""

        async def single_nested():
            async def inner():
                for i in [10, 20, 30]:
                    yield i
    ...
    @pytest.mark.asyncio
    async def test_collect_stream_basic(self):
        """Test basic collect_stream functionality."""
        collect = collect_stream()
        assert collect.name == "collect"

        input_stream = async_range(4)
        result_stream = collect(input_stream)
        values = [item async for item in result_stream]

        # Should yield a single list containing all items
        assert len(values) == 1
        assert values[0] == [0, 1, 2, 3]

    @pytest.mark.asyncio
    async def test_collect_stream_empty(self):
        """Test collect_stream with empty stream."""
        collect = collect_stream()
    ...
    @pytest.mark.asyncio
    async def test_collect_stream_composition(self):
        """Test collect_stream in composition with other combinators."""

        # Create a pipeline that processes and then collects
        pipeline = compose(
            filter_stream(is_even), compose(map_stream(double), collect_stream())
        )

        input_stream = async_range(6)  # [0,1,2,3,4,5]
        result_stream = pipeline(input_stream)
        values = [item async for item in result_stream]

        # Filter evens: [0,2,4], double: [0,4,8], collect: [[0,4,8]]
        assert len(values) == 1
        assert values[0] == [0, 4, 8]


class TestNewCombinatorsIntegration:
    """Integration tests for new combinators with existing ones."""
    ...
    @pytest.mark.asyncio
    async def test_guard_with_recovery_pattern(self):
        """Test guard_stream combined with error recovery patterns."""

        # Use filter to avoid guard failures instead of guard itself
        safe_pipeline = compose(
            filter_stream(is_positive),  # Remove problematic values
            guard_stream(lambda x: x < 10, "Must be less than 10"),  # Guard remaining
        )
        ...
        # Filter removes 0, guard passes [1,2,3,4]
        assert values == [1, 2, 3, 4]

    @pytest.mark.asyncio
    async def test_memoize_with_then_side_effects(self):
        """Test memoize_stream combined with then_stream for side effect tracking."""
        ...
        # Pipeline: memoize -> track calls -> double values
        pipeline = compose(
            memoize_stream(), compose(then_stream(track_calls), map_stream(double))
        )
        ...
        # Values: memoized [1,2,1,3,2] -> doubled [2,4,2,6,4]
        assert values == [2, 4, 2, 6, 4]
        # Side effects should track all values (including memoized ones)
        assert call_count == [1, 2, 1, 3, 2]

    @pytest.mark.asyncio
    async def test_flatten_with_collect_nested_processing(self):
        """Test flatten_stream combined with collect_stream for nested processing."""

        async def create_grouped_streams():
            """Create streams grouped by some criteria."""
        ...
        # Pipeline: flatten -> double -> collect
        pipeline = compose(
            flatten_stream(), compose(map_stream(double), collect_stream())
        )

        nested_stream = create_grouped_streams()
        result_stream = pipeline(nested_stream)
        values = [item async for item in result_stream]

        # Flatten: [0,2,4,1,3,5], double: [0,4,8,2,6,10], collect: [[0,4,8,2,6,10]]
        assert len(values) == 1
        assert values[0] == [0, 4, 8, 2, 6, 10]

    @pytest.mark.asyncio
    async def test_while_condition_with_complex_transform(self):
        """Test while_condition_stream with complex transformation flow."""

        # Transform: filter evens -> double -> add 1
        complex_transform = compose(
            filter_stream(is_even), compose(map_stream(double), map_stream(increment))
        )

        # Apply transform while values are small
        conditional_pipeline = while_condition_stream(
            lambda x: x < 4, complex_transform
        )

        input_stream = async_range(6)  # [0,1,2,3,4,5]
        result_stream = conditional_pipeline(input_stream)
        values = [item async for item in result_stream]

        # 0: even->double->inc = 1, 1: odd->filtered out = nothing yielded
        # 2: even->double->inc = 5, 3: odd->filtered out = nothing yielded
        # 4: x>=4, condition false, pass through 4,5 unchanged
        # So we expect: [1, 5, 4, 5] - but let's trace this carefully

        # Actually, let me reconsider the while_condition_stream logic...
        # It should stop when condition becomes false, then pass remaining unchanged
        # 0 < 4: transform 0 (even) -> 0*2+1 = 1
        # 1 < 4: transform 1 (odd, filtered out) -> no output for this item
        # 2 < 4: transform 2 (even) -> 2*2+1 = 5
        # 3 < 4: transform 3 (odd, filtered out) -> no output for this item
